...
# glimpse() function is in dplyr package. First we had to use 
# install.packages("dplyr") in the console to download the package. 
# Then we use library(dplyr) to load the package. After that, we can use 
# glimpse() to see each variable and the first several observations.

library(dplyr)
glimpse(mtcars)

# skim() is in skimr package. After downloading it (like with dplyr), 
# we can load the package and use the skim() function.
...
# Use help to learn about data. Then find the oldest tree and the shortest tree
