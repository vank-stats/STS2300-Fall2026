...
top <- ggplot(diamonds, aes(x = price)) +
...
majors <- read.csv("https://raw.githubusercontent.com/vank-stats/STS2300-Fall2026/refs/heads/main/Data/MTH_STS_Majors_long.csv")

ggplot(majors, aes(y = Major, x = Count)) +
  geom_boxplot(fill = "#b59a57", color = "#73000a", alpha = 0.5) +
...
ggplot(majors, aes(x = Year, y = Count, color = Major)) +
  ...
  labs(title = "Elon MTH/STS Department Majors", 
       ...
       y = "Number of Majors") +
  scale_color_manual(values = c("#b7410e", "#527b82", "#d3dfd3", "#788a38"),
                     labels = c("Applied/nMath", "Data/nAnalytics", 
                                "Math", "Stats")) +
  scale_x_continuous(breaks = seq(2007, 2025, 3)) +
  theme(plot.background = element_rect(fill = "#73000a"),
        panel.background = element_rect(fill = "white"),
        panel.grid = element_line(color = "lightgray"),
        plot.title = element_text(color = "#b59a57", 
                                  size = 16, 
                                  face = "bold", 
                                  family = "Courier"),
        axis.title = element_text(color = "#b59a57", 
                                  size = 14),
        axis.text = element_text(color = "#b59a57"),
        axis.ticks = element_line(color = "#b59a57"),
...
# Practice: Choose another dataset to explore. Create at least three graphs
#  (with at least one using scale and theme options). Combine them like above.
